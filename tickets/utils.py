...
# ۴ ثانیه بود، از زمانی که این سرویس کاملاً هنگ کرده بود و هر تماس یک نخ
# گانیکورن را تا آخر تایم‌اوت قفل نگه می‌داشت. حالا که سرویس برگشته،
# اندازه‌گیری روی ترافیک واقعی نشان می‌دهد پاسخ‌های موفق میانه‌ی ۰.۰۱ و
# p99 برابر ۰.۸۲ ثانیه دارند -- یعنی ۸ ثانیه فقط سقفِ ایمنی است و در عمل
# هیچ‌وقت به آن نمی‌رسیم، ولی جلوی قطع‌شدنِ استعلامِ کاربرانی را می‌گیرد
# که اتفاقاً یک تماسِ کندتر از معمول خورده‌اند.
FAN_API_TIMEOUT = 8
...
# را باز کند.
FAN_API_FAIL_THRESHOLD = 30
FAN_API_OPEN_SECONDS = 20
