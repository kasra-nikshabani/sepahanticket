...
# این سرویس ناپایدار است: در بازه‌هایی به‌سرعت جواب می‌دهد و در بازه‌هایی
# کاملاً می‌خوابد (TCP وصل می‌شود ولی پاسخی نمی‌آید). یک بار ۸ ثانیه‌اش
# کردیم چون در لحظه‌ی اندازه‌گیری سالم به‌نظر می‌رسید -- ولی وقتی دوباره
# افتاد، همان ۸ ثانیه یعنی هر تماسِ ناموفق دو برابرِ قبل یک نخ گانیکورن را
# قفل نگه می‌داشت و کندیِ کل سایت برگشت. عدد باید برای بدترین حالت انتخاب
# شود، نه بهترین حالت: پاسخ‌های سالم زیر یک ثانیه‌اند، پس ۴ ثانیه هم برای
# آن‌ها کافی است و هم هزینه‌ی خرابی را نصف می‌کند.
FAN_API_TIMEOUT = 4
...
# مدار را می‌بندد.
#
# ===== چرا این اعداد بزرگ شدند =====
# مقادیر اولیه (۵ خطا -> ۶۰ ثانیه قطع) برای وضعیتی تنظیم شده بودند که
# سرویس کاملاً مرده بود. وقتی سرویس برگشت، همان تنظیم بیش‌ازحد حساس شد:
# روی ۱۳۶۷ استعلامِ موفق، ۲۵۲۰ پاسخِ ۵۰۳ دادیم -- یعنی چند تماسِ کندِ
# پراکنده کافی بود که مدار باز شود و یک دقیقه‌ی کامل، استعلامِ همه‌ی
# کاربران قطع شود. مدارشکن باید آخرین خط دفاع باشد، نه یک تله‌ی زودرس:
# آستانه بالا رفت و مدت قطع کوتاه شد تا فقط یک خرابیِ واقعی و پیوسته آن
# را باز کند.
FAN_API_FAIL_THRESHOLD = 12
FAN_API_OPEN_SECONDS = 30
...
def record_fan_api_failure():
    ...
    شمارش با incr انجام می‌شود که در Redis اتمیک است؛ الگوی قبلی
    (get سپس set) زیر بار بالا با ده‌ها نخِ هم‌زمان، شمارش را از دست
    می‌داد و نتیجه‌اش غیرقابل‌پیش‌بینی بود.
    """
    try:
        try:
            fails = cache.incr(_FAN_FAIL_KEY)
        except ValueError:
            # کلید وجود ندارد -- اولین خطای این پنجره
            cache.set(_FAN_FAIL_KEY, 1, timeout=FAN_API_OPEN_SECONDS)
            fails = 1
