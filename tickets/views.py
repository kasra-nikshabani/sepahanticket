...
def get_age_from_jalali(dob_str):
    """سن از تاریخ تولد شمسی. ورودیِ نامعتبر -> ۱۰۰ (یعنی «نامشخص»).

    ===== چرا سن منفی باید گرفته شود =====
    اگر کاربر تاریخ تولدی در آینده وارد کند، این محاسبه عدد منفی می‌دهد و
    دو چیز خراب می‌شود، هر دو بعد از اینکه پول از حساب کاربر کم شده:
      ۱. سن منفی از ۱۵ کمتر است، پس بلیط «رایگان» حساب می‌شود.
      ۲. ستون age در دیتابیس قید age >= 0 دارد، پس درج بلیط رد می‌شود --
         verify زیبال موفق بوده، پول گرفته شده، ولی هیچ بلیطی صادر نمی‌شود
         و تراکنش «ناموفق» علامت می‌خورد. (روی داده‌ی واقعی دیده شد.)
    تاریخ آینده مثل هر ورودیِ نامعتبر دیگری ۱۰۰ برگردانده می‌شود: نه
    رایگان، و سازگار با قید دیتابیس.
    """
    try:
        parts = dob_str.split('/')
        if len(parts) == 3:
