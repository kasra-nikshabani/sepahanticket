...
class Payment(models.Model):
    ...
    # «از درگاه پرسیدیم» -- جدا از «پول گرفته شده بود».
    # بدون این، پرداختی که استعلامش جواب «پرداخت نشده» داده همچنان در دامنه‌ی
    # جاروکش می‌ماند و هر ده دقیقه دوباره پرسیده می‌شود؛ با ۲۲ هزار رکورد یعنی
    # تایمر هیچ‌وقت از ۴۰۰ تای اول جلوتر نمی‌رود و بی‌دلیل به درگاه فشار می‌آورد.
    gateway_checked_at = models.DateTimeField(
        null=True, blank=True, db_index=True,
        help_text='آخرین باری که وضعیت این پرداخت از درگاه استعلام شد (نتیجه هرچه بوده)',
    )
