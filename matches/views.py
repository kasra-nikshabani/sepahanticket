...
def select_block(request, match_id):
    ...
    # فعال/غیرفعال بودن بلوک مخصوصِ همین مسابقه است، نه گلوبال
    blocks = Block.objects.filter(
        stadium=stadium,
        id__in=get_active_block_ids(match),
...
def show_block_map(request, match_id):
    ...
    block = get_object_or_404(Block, id=block_id)
    # ===== بلوکی که برای همین مسابقه غیرفعال شده نباید قابل خرید باشد، حتی
    # اگر شناسه‌اش از سشن/لینک قدیمی بیاید =====
    if not is_block_active_for_match(match, block):
        messages.error(request, 'این بلوک برای این مسابقه در دسترس نیست.')
        return redirect('matches:select_block', match_id=match_id)

    active_row_ids = get_active_row_ids(match, block=block)
    rows = Row.objects.filter(block=block, id__in=active_row_ids).order_by('number')

    # بارگذاری یک‌جا: همه صندلی‌ها + MatchSeatهای موجود (بدون get_or_create در حلقه)
    rows = list(rows)
    all_seats = list(
        Seat.objects
        .filter(row__block=block, row_id__in=active_row_ids)
        .select_related('row')
    ...
    for row in rows:
        ...
        for seat in row_seats:
            ...
            if not ms:
                continue
            # صندلی‌ای که غیرفعال است هیچ‌وقت سبز نشان داده نمی‌شود -- مستقل
            # از وضعیت رزرو/فروش این مسابقه. دو حالت: یا به‌صورت گلوبال
            # غیرفعال است (Seat.is_available)، یا ادمین آن را فقط برای همین
            # مسابقه خاموش کرده (MatchSeat.is_enabled).
            if not seat.is_available or not ms.is_enabled:
                seat_list.append({
                    'number': seat.number,
                    'is_available': False,
                    'id': ms.id,
...
def select_row(request, match_id):
    ...
    if not is_block_active_for_match(match, block):
        messages.error(request, 'این بلوک برای این مسابقه در دسترس نیست.')
        return redirect('matches:select_block', match_id=match_id)
    rows = Row.objects.filter(block=block, id__in=get_active_row_ids(match, block=block)).order_by('number')
...
def _compute_match_report_stats(match):
    ...
    """
    from tickets.models import Order, DiscountCode, SpecialCode
    ...
    organic_sold_qs = sold_tickets_qs.filter(discount_code__isnull=True, special_code__isnull=True)
    ...
    # ===== توجه: هر ۵ دسته‌ی زیر (قیمت کامل/باسا/رایگان‌زیر۱۵/تخفیف‌ویژه/کد
    # ویژه) با هم کاملاً مانع‌الجمع (mutually exclusive) و جمعشون برابرِ
    # sold_tickets_qs.count() هست -- یعنی هر بلیطِ خریداری‌شده دقیقاً توی
    # یکی از این ۵ دسته می‌افته، نه بیشتر. =====
    not_special = Q(special_code__isnull=True)
    category_stats = sold_tickets_qs.aggregate(
        free_age_count=Count('id', filter=Q(price=0) & Q(discount_code__isnull=True) & not_special),
        free_age_used_count=Count('id', filter=Q(price=0) & Q(discount_code__isnull=True) & not_special & Q(is_used=True)),
        basa_discount_count=Count('id', filter=Q(basa_discount_amount__gt=0) & Q(discount_code__isnull=True) & not_special),
        basa_discount_total=Sum('basa_discount_amount', filter=Q(basa_discount_amount__gt=0) & Q(discount_code__isnull=True) & not_special),
        basa_revenue=Sum('price', filter=Q(basa_discount_amount__gt=0) & Q(discount_code__isnull=True) & not_special),
        full_price_count=Count('id', filter=~Q(price=0) & Q(basa_discount_amount=0) & Q(discount_code__isnull=True) & not_special),
        full_price_revenue=Sum('price', filter=~Q(price=0) & Q(basa_discount_amount=0) & Q(discount_code__isnull=True) & not_special),
        discount_code_count=Count('id', filter=Q(discount_code__isnull=False) & not_special),
        discount_code_used_count=Count('id', filter=Q(discount_code__isnull=False) & not_special & Q(is_used=True)),
        discount_code_revenue=Sum('price', filter=Q(discount_code__isnull=False) & not_special),
        discount_code_amount_given=Sum('discount_code_amount', filter=Q(discount_code__isnull=False) & not_special),
        special_code_ticket_count=Count('id', filter=Q(special_code__isnull=False)),
        special_code_ticket_used_count=Count('id', filter=Q(special_code__isnull=False) & Q(is_used=True)),
    )
    ...
    # ===== کد ویژه -- تعداد کدهای ساخته‌شده برای این مسابقه (چه استفاده‌شده
    # چه نه)، در برابر تعداد بلیط‌هایی که واقعاً با یکی از این کدها گرفته
    # شدن، و از بین اون‌ها چندتاش از گیت عبور کرده. =====
    special_codes_created_count = SpecialCode.objects.filter(match=match).count()
    special_code_ticket_count = category_stats['special_code_ticket_count']
    special_code_ticket_used_count = category_stats['special_code_ticket_used_count']
    special_code_ticket_not_used_count = special_code_ticket_count - special_code_ticket_used_count
    special_code_ticket_used_percent = round(
        (special_code_ticket_used_count / special_code_ticket_count * 100) if special_code_ticket_count > 0 else 0, 1
    )
    special_code_ticket_not_used_percent = round(
        100 - special_code_ticket_used_percent, 1
    ) if special_code_ticket_count > 0 else 0
    ...
    blocks = Block.objects.filter(stadium=match.stadium).filter(
        Q(id__in=active_block_ids) | Q(id__in=blocks_with_sales)
    ).order_by('order')
    blocks_data = []
    block_price_map = get_block_price_map(match)
    for block in blocks:
        ...
        blocks_data.append({
            ...
            'row_count': Row.objects.filter(block=block, id__in=active_row_ids).count(),
    ...
    return {
        ...
        'special_codes_created_count': special_codes_created_count,
        'special_code_ticket_count': special_code_ticket_count,
        'special_code_ticket_used_count': special_code_ticket_used_count,
        'special_code_ticket_not_used_count': special_code_ticket_not_used_count,
        'special_code_ticket_used_percent': special_code_ticket_used_percent,
        'special_code_ticket_not_used_percent': special_code_ticket_not_used_percent,
...
def admin_match_full_report_excel(request, match_id):
    ...
    overview_rows = [
        ...
        ('کد ویژه ساخته‌شده', stats['special_codes_created_count']),
        ('کد ویژه استفاده‌شده (بلیط گرفته‌شده)', stats['special_code_ticket_count']),
        ('بلیطِ کد ویژه که از گیت عبور کرده', stats['special_code_ticket_used_count']),
    ]
    ...
    ws_total.append(['کد ویژه (رایگان‌کننده‌ی یک بلیط)', stats['special_code_ticket_count']])
    ...
    ws_vip.append([
        'کد ویژه (رایگان‌کننده‌ی یک بلیط)', stats['special_code_ticket_count'], stats['special_code_ticket_used_count'],
...
# این سه ویو قبلاً وضعیت را روی خودِ بلوک/صندلی (که متعلق به ورزشگاه‌اند و
# بین همه‌ی مسابقات مشترک) می‌نوشتند و همین باعث می‌شد تغییر برای یک بازی
# روی همه‌ی بازی‌ها اثر بگذارد. بدتر: _sync_seats_to_blocks_status با یک
# UPDATE دسته‌جمعی، is_available تمام صندلی‌های بلوک را صفر می‌کرد و اگر
# بعداً بلوک از مسیر دیگری فعال می‌شد، صندلی‌ها خاموش می‌ماندند -- که در
# عمل ۳۴هزار صندلی را غیرقابل‌فروش کرده بود.
#
# حالا تنها جای فعال/غیرفعال کردن، صفحه‌ی «چیدمان ورزشگاه این مسابقه»
# (جزئیات مسابقه) است که همه‌چیز را مخصوصِ همان مسابقه ذخیره می‌کند.
# خودِ ویوها حذف نشدند تا لینک/بوکمارک قدیمی ۵۰۰ ندهد؛ فقط کاری نمی‌کنند
# و کاربر را به مسیر درست راهنمایی می‌کنند.

_MOVED_MSG = (
    'فعال/غیرفعال کردن بلوک، ردیف و صندلی از این صفحه برداشته شده است. '
    'این کار را از «جزئیات مسابقه ← چیدمان ورزشگاه این مسابقه» انجام دهید '
    'تا تغییر فقط روی همان مسابقه اعمال شود و به بقیه‌ی مسابقات سرایت نکند.'
)


@staff_member_required
def toggle_block_status(request, block_id):
    messages.warning(request, _MOVED_MSG)
    return redirect('matches:admin_block_list')
...
def admin_bulk_toggle_floor(request, stadium_id, floor):
    messages.warning(request, _MOVED_MSG)
    return redirect(f"{reverse('matches:admin_block_list')}?stadium_id={stadium_id}")
...
# ============================================================
#  چیدمان ورزشگاه مخصوصِ یک مسابقه (فعال/غیرفعال کردن بلوک/ردیف/صندلی)
# ============================================================
# همه‌ی تغییرات اینجا فقط روی همین مسابقه اثر می‌گذارند و به مسابقات دیگرِ
# همان ورزشگاه سرایت نمی‌کنند -- برخلاف صفحه‌ی «مدیریت سکوها» که وضعیت
# پیش‌فرضِ گلوبال را عوض می‌کند.

@staff_member_required
def admin_match_stadium_layout(request, match_id):
    """لیست بلوک‌های ورزشگاه با وضعیت فعال/غیرفعالِ مخصوصِ همین مسابقه."""
    match = get_object_or_404(Match, id=match_id)
    active_map = get_block_active_map(match)
    override_ids = set(
        MatchBlockActive.objects.filter(match=match).values_list('block_id', flat=True)
    )

    blocks = Block.objects.filter(stadium=match.stadium).order_by('floor', 'order')
    sold_by_block = {
        row['seat__row__block_id']: row['c']
        for row in (
            Ticket.objects.filter(
                match=match, status__in=['paid', 'admin_assigned', 'vip_issued']
            ).values('seat__row__block_id').annotate(c=Count('id'))
        )
    }
...
@staff_member_required
def admin_match_block_layout(request, match_id, block_id):
    """ردیف‌ها و صندلی‌های یک بلوک، با وضعیت مخصوصِ همین مسابقه."""
    match = get_object_or_404(Match, id=match_id)
    block = get_object_or_404(Block, id=block_id, stadium=match.stadium)

    row_active_map = get_row_active_map(match, block=block)
    row_override_ids = set(
    ...
    ms_map = {
        ms.seat_id: ms
        for ms in MatchSeat.objects.filter(match=match, seat__row__block=block)
    }
    sold_seat_ids = set(
        Ticket.objects.filter(
            match=match, seat__row__block=block,
    ...
    seats_by_row = {}
    for seat in seats:
        ms = ms_map.get(seat.id)
        seats_by_row.setdefault(seat.row_id, []).append({
            'seat': seat,
            # اگر هنوز MatchSeat ساخته نشده، پیش‌فرض یعنی فعال
            'is_enabled': ms.is_enabled if ms else True,
            'global_available': seat.is_available,
...
@staff_member_required
def admin_match_toggle_block(request, match_id, block_id):
    """فعال/غیرفعال کردن یک بلوک فقط برای همین مسابقه."""
    if request.method != 'POST':
        return redirect('matches:admin_match_stadium_layout', match_id=match_id)
    match = get_object_or_404(Match, id=match_id)
    block = get_object_or_404(Block, id=block_id, stadium=match.stadium)
    ...
    messages.success(
        request,
        f'بلوک «{block.name}» فقط برای این مسابقه {"فعال" if new_state else "غیرفعال"} شد.'
    )
    return redirect('matches:admin_match_stadium_layout', match_id=match.id)


@staff_member_required
def admin_match_toggle_row(request, match_id, row_id):
    """فعال/غیرفعال کردن یک ردیف فقط برای همین مسابقه."""
    if request.method != 'POST':
        return redirect('matches:admin_match_stadium_layout', match_id=match_id)
    match = get_object_or_404(Match, id=match_id)
    ...
    messages.success(
        request,
        f'ردیف {row.number} از بلوک «{row.block.name}» فقط برای این مسابقه '
        f'{"فعال" if new_state else "غیرفعال"} شد.'
    )
    return redirect('matches:admin_match_block_layout', match_id=match.id, block_id=row.block_id)


@staff_member_required
def admin_match_toggle_seat(request, match_id, seat_id):
    """فعال/غیرفعال کردن یک صندلی فقط برای همین مسابقه."""
    if request.method != 'POST':
        return redirect('matches:admin_match_stadium_layout', match_id=match_id)
    match = get_object_or_404(Match, id=match_id)
    ...
    messages.success(
        request,
        f'صندلی {seat.number} (ردیف {seat.row.number}) فقط برای این مسابقه '
        f'{"فعال" if match_seat.is_enabled else "غیرفعال"} شد.'
    )
    return redirect('matches:admin_match_block_layout', match_id=match.id, block_id=seat.row.block_id)


@staff_member_required
def admin_match_reset_block(request, match_id, block_id):
    """حذف تنظیم اختصاصیِ این مسابقه برای یک بلوک -- برگشت به وضعیت پیش‌فرض گلوبال."""
    if request.method != 'POST':
        return redirect('matches:admin_match_stadium_layout', match_id=match_id)
    match = get_object_or_404(Match, id=match_id)
    block = get_object_or_404(Block, id=block_id, stadium=match.stadium)
    ...
    if deleted:
        messages.success(request, f'بلوک «{block.name}» به وضعیت پیش‌فرض برگشت.')
    return redirect('matches:admin_match_stadium_layout', match_id=match.id)
